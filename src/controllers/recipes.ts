...
export async function create(formData: FormData){
    ...
    if (!title || !description || !prepTime || !cookTime || !ingredients || !instructions || !toolsNeeded || !userIdString) {
  ...
  const userId = parseInt(userIdString, 10);
    const recipe = await prisma.recipe.create({
        data: {
          ...
          difficulty: prepTime, // If prepTime is being reused as difficulty here, change this later
